...
def test_create_product(admin_token):
    product_api = AdminAPI (base_url=BASE_URL, token=admin_token)
    response = product_api.create_product("Test Product")
    assert response["name"] == "Test Product"


@pytest.mark.usefixtures("admin_token")
def test_get_current_product_count(admin_token):
    product_api = ProductAPI(base_url=BASE_URL, token=admin_token)
